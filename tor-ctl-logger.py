...
def main():
    ...
    parser.add_argument('-p', '--port',
        ...
        dest="ctlport", required=False, default=None)

    parser.add_argument('-s', '--socket',
        help="""Path to the Tor control socket""",
        action="store", type=str, metavar="PATH",
        dest="ctlsocket", required=False, default=None)
    ...
    if args.ctlport == None and args.ctlsocket == None:
        print("Please specify either a control port or control socket. See 'tor-ctl-logger --help' for more info.")
        return
...
def run(args):
    with open(args.logpath, 'a') as logfile:
        ctl_loc = args.ctlsocket if args.ctlsocket != None else args.ctlport
        startup_msg = "started tor-ctl-logger on {0}, logging events to {1}\n".format(ctl_loc, args.logpath)
        __log(logfile, startup_msg)
        __log(sys.stderr, startup_msg)

        if args.ctlsocket != None:
            with stem.control.Controller.from_socket_file(path = args.ctlsocket) as torctl:
                __run_helper(args, logfile, torctl)
        elif args.ctlport != None:
            with stem.control.Controller.from_port(port = args.ctlport) as torctl:
                __run_helper(args, logfile, torctl)

def __run_helper(args, logfile, torctl):
    torctl.authenticate()
...
def __log(logfile, msg):
    ...
    print("{0} {1:.02f} {2}".format(now.strftime("%Y-%m-%d %H:%M:%S"), unix_ts, msg).strip(), file=logfile)
